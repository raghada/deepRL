...
class Environment:
    ...
    def show_greedy(self, agent):
            ...
            for step_num in range(100):
                ...
                next_state, distance_to_goal = self.step(state, action)
                path.append(next_state)
                state = next_state
            path = np.array(path)
            #print(path)
            path[:,1] = 1 - path[:,1]
            cv2.polylines(self.image,[(self.magnification*path).astype(np.int32)],False,(0,255,255),thickness=2)
            # Show the image
            cv2.imshow("Environment", self.image)
            # This line is necessary to give time for the image to be rendered on the screen
            cv2.waitKey(1)